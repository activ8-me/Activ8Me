    ...
    return (
      <View style={styles.container}>
        <View style={{  marginBottom:100,
          ...
          <Text style={styles.title2}>wakey wakey, sleepy head!</Text>
          <View style={{display:'flex', flexDirection:'row', alignContent: 'center'}}>
              <Text style={styles.clock}>{this.state.time ? `${this.state.time.slice(0,2)} : ${this.state.time.slice(3,5)}` : `${moment().format('LT').slice(0,2)} : ${moment().format('LT').slice(3,5)}`}</Text>
              <Text style={styles.meridiem}>{this.state.time ? this.state.time.slice(6) : moment().format('LT').slice(6)}</Text>
            </View>
        </View>
        {
